/**
 * Vanilla port of Canvas UI DecryptReveal, with html-to-image fallback
 * when the experimental HTML-in-canvas API is missing.
 */
...
export function createDecryptReveal(elements, options = {}) {
  ...
  function uploadContent() {
    ...
    if (!htmlInCanvas && !hasFallback) return;
    contentDirty = false;
    cellsDirty = true;
  ...
  function render() {
    ...
    const live = htmlInCanvas || hasFallback;
    gl.uniform1f(u.uCrisp, reducedMotion || !live ? 1 : 0);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  ...
  function frame(now) {
    ...
    const live = htmlInCanvas || hasFallback;
    if (settled && !contentDirty && (reducedMotion || !live || !churning)) {
...
function themeBackground() {
  ...
  if (scheme === "light") return "#ffffff";
  if (scheme === "dark") return "#000000";
  return window.matchMedia("(prefers-color-scheme: dark)").matches
    ? "#000000"
    : "#ffffff";
}

async function captureContent(node) {
  const api = window.htmlToImage;
  if (!api?.toCanvas) throw new Error("html-to-image unavailable");
  try {
    if (document.fonts?.ready) await document.fonts.ready;
  } catch {
    /* ignore */
  }
  const cssWidth = Math.max(1, Math.round(node.offsetWidth));
  const cssHeight = Math.max(1, Math.round(node.offsetHeight));
  const pixelRatio = Math.min(window.devicePixelRatio || 1, 2);
  return api.toCanvas(node, {
    pixelRatio,
...
export function mountDecryptReveal(root, options = {}) {
  if (!root) return null;
  const source = root.querySelector(".decrypt-reveal__source");
  const content = root.querySelector(".decrypt-reveal__content");
  const output = root.querySelector(".decrypt-reveal__output");
  ...
  const instance = createDecryptReveal(
    ...
    { ...options, background: themeBackground() },
  );
  ...
  async function runCapture() {
    if (native || capturing) return;
    capturing = true;
    root.classList.remove("decrypt-reveal--active");
    try {
      const canvas = await captureContent(content);
      instance.adoptCapture(canvas);
      root.classList.add("decrypt-reveal--active");
    } catch (err) {
      console.warn("DecryptReveal capture failed:", err);
      root.classList.remove("decrypt-reveal--active");
    } finally {
      capturing = false;
    }
  ...
  function scheduleCapture() {
    ...
    captureTimer = window.setTimeout(runCapture, 180);
  }
  ...
  const themeObs = new MutationObserver(() => {
    instance.setOptions({ background: themeBackground() });
    scheduleCapture();
  });
...
function boot() {
  document.querySelectorAll("[data-decrypt-reveal]").forEach((root) => {
    mountDecryptReveal(root, {
      radius: 400,
      softness: 0.5,
      cell: 10,
      aspect: 0.75,
